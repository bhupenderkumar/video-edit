...
export async function processVideo(projectId: string): Promise<void> {
  ...
  const profile = store.getProfile(project.profile_id || "default");
  ...
  try {
    // Update status
    store.updateProject(projectId, { status: "processing", processing_started_at: new Date().toISOString() });
    ...
    store.updateProject(projectId, { status: "extracting" });
    ...
    // Step 4: Transcribe
    store.updateProject(projectId, { status: "transcribing" });
    const transcript = await transcribeAudio(audioPath);
    store.updateProject(projectId, { transcript: JSON.stringify(transcript) });
    ...
    store.updateProject(projectId, { frame_analysis: JSON.stringify(frameAnalysis) });
    ...
    store.updateProject(projectId, { edit_plan: JSON.stringify(editPlan) });

    // Step 7: Render
    store.updateProject(projectId, { status: "rendering" });
    ...
    store.updateProject(projectId, { status: "failed", error_message: message });
    throw err;
  } finally {
    // Cleanup temp files
